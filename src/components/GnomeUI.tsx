...
const CurrentUserDisplay = () => {
	...
	if (!user)
		return <div className="text-sm text-gray-400 italic">Не авторизован</div>;
	return (
...
const UsersList = () => {
	...
	const handleReportClick = (u: string) => {
		if (u === user) {
			window.location.href = "/successJournal";
		} else {
			setTargetUser(u);
			setIsAuthOpen(true);
		}
	...
	return (
		<>
						...
						<div className="flex items-center gap-2">
							<div
								className={cn(
									...
									u === user ? "bg-ubuntu-orange" : "bg-gray-400",
								)}
							/>
							{u}
						</div>
						<button
							onClick={() => handleReportClick(u)}
			...
			<AuthModal
				...
				onSuccess={() => (window.location.href = "/successJournal")}
				title="Доступ к отчёту"
				initialNickname={targetUser}
			/>
		</>
	);
};
export const AuthModal: React.FC<{
	...
	initialNickname = "",
		...
		const handleSubmit = async (e: React.FormEvent) => {
			e.preventDefault();
			setError("");
			const result = await login(nickname, password, needsDescription ? description : undefined);

			if (result.requiresDescription) {
				setNeedsDescription(true);
				setError("Пользователь не найден. Пожалуйста, заполните описание для регистрации.");
		...
		return (
			<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
				<div className="gnome-window w-full max-w-md animate-in zoom-in duration-200">
					<div className="gnome-header flex h-[44px] items-center justify-between px-4">
						<div className="w-8" />
						<span className="text-sm font-bold text-gray-600">{title}</span>
						<button
							onClick={onClose}
							className="flex w-8 justify-end text-gray-400 hover:text-gray-600"
						>
							✕
						</button>
					</div>
						...
						<form onSubmit={handleSubmit} className="space-y-3">
							<div>
								...
								<input
									...
									className="w-full px-3 py-1 border border-gray-300 rounded-md focus:ring-2 focus:ring-ubuntu-orange outline-none disabled:bg-gray-50 disabled:text-gray-500 text-sm"
									placeholder="Только буквы, без цифр в начале"
									required
									disabled={!!initialNickname || needsDescription}
								/>
							</div>
							<div>
								<label className="block text-xs font-bold text-gray-500 uppercase mb-1">
								...
								<input
									...
									className="w-full px-3 py-1 border border-gray-300 rounded-md focus:ring-2 focus:ring-ubuntu-orange outline-none text-sm"
									required
									disabled={needsDescription}
								/>
							</div>
							...
							{needsDescription && (
								<div className="animate-in fade-in slide-in-from-top-2 duration-300">
									...
									<textarea
										...
										className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-ubuntu-orange outline-none text-sm resize-none h-20"
										placeholder="Расскажите немного о себе, чтобы завершить регистрацию..."
										required
									/>
								</div>
							)}
							...
							{error && (
								<div className="text-red-500 text-xs font-medium bg-red-50 p-2 rounded border border-red-100">
									{error === "Неверный пароль"
										? "Неверный пароль. Попробуйте другой никнейм или пароль."
										: error}
								</div>
							)}
							<button
								type="submit"
								className="w-full bg-ubuntu-orange text-white text-[12px] py-1.5 rounded-md hover:bg-[#ff632d] transition-colors shadow-md"
							>
								{needsDescription ? "Завершить регистрацию" : "Войти / Начать"}
							</button>
						</form>
					</div>
				</div>
...
export const TopBar: React.FC = () => {
	...
	const [authTarget, setAuthTarget] = useState<
		"/knowledgeJourney" | "/successJournal"
	>("/knowledgeJourney");
	...
	return (
		<>
			...
			<AuthModal
				...
				title={
					authTarget === "/successJournal"
						? "Вход в журнал успехов"
