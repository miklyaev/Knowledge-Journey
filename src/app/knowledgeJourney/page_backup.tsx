...
const KnowledgeJourney = () => {
	...
	// Recommended sections states
	const [recommendedSections, setRecommendedSections] = useState<string[]>([]);
	const [isRecommendedSectionsEnabled, setIsRecommendedSectionsEnabled] = useState(false);
	const [selectedRecommendedSection, setSelectedRecommendedSection] = useState<string | null>(null);
	const [pendingRecommendedSectionTitle, setPendingRecommendedSectionTitle] = useState<string | null>(null);
	...
	// Автоматическая загрузка RAG данных при смене темы
	useEffect(() => {
		const fetchRagData = async () => {
			if (selectedTopicId === 'none' || !isSourceEnabled) {
				setPdfId(null);
				setSections([]);
			...
			const baseUrl = getPublicApiBaseUrl();
			try {
				const response = await fetch(`${baseUrl}/api/pdf/by-theme/${selectedTopicId}`);
				if (response.ok) {
					const data = await response.json();
					setPdfId(data.id);
		...
		fetchRagData();
	}, [selectedTopicId, isSourceEnabled]);
