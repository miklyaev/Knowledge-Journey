...
app.post('/api/gigachat/generate', async (req, res) => {
	try {
		const { prompt, systemPrompt = "Ты сеньор разработчик на C\\C++, имеешь большой опыт в программировании микроконтроллеров Arduino, знаешь большинство библиотек для работы с этим контроллером. Всегда выдавай один и самый простой вариант" } = req.body;
...
app.post('/api/yandexgpt/generate', async (req, res) => {
	try {
		const {
			prompt,
			systemPrompt = "Ты эксперт по C++ и микроконтроллерам Arduino. Пиши кратко, по делу, без лишних объяснений."
		} = req.body;
